...
def generate_launch_description():
    ...
    frame_prefix_arg = DeclareLaunchArgument("frame_prefix", default_value="echo/")
    ...
    namespace_arg = DeclareLaunchArgument("namespace", default_value=TextSubstitution(text="echo"))
    ...
    gcs_url_arg = DeclareLaunchArgument("gcs_url", default_value=TextSubstitution(text=""))
    ...
    return LaunchDescription(
        [
                ...
                actions=[
                    ...
                    SetParametersFromFile(
                        filename=PathJoinSubstitution(
                            [FindPackageShare("echoboat_project11"), "config", "echo.yaml"]
                        )
                    ),
                    SetParametersFromFile(
                        filename=PathJoinSubstitution(
                            [FindPackageShare("echoboat_project11"), "config", "sim_echo.yaml"]
                        ),
                    ...
                    SetParameter(name="sea_surface_frame", value=[frame_prefix, "map_tide"]),
                    ...
                    IncludeLaunchDescription(
                        PythonLaunchDescriptionSource(
                            PathJoinSubstitution(
                                [FindPackageShare("echo_helm"), "launch", "echo_helm_launch.py"]
                            )
                        ),
                    ),
                    GroupAction(
                        actions=[
                            ...
                            SetParameter(name="map_frame", value=[frame_prefix, "map"]),
                            IncludeLaunchDescription(
                                PythonLaunchDescriptionSource(
                                    PathJoinSubstitution(
                                        [
            ...
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    PathJoinSubstitution(
                        [FindPackageShare("echoboat_project11"), "launch", "nav2_bringup_launch.py"]
                    )
                ),
                launch_arguments={
                    "namespace": namespace,
...
#     <rosparam command="load" file="$(find echoboat_project11)/config/echo.yaml"/>  # noqa: E501

#     <param name="udp_bridge/remotes/operator/connections/default/host" value="$(arg operator_host)"/>  # noqa: E501
